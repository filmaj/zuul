...
SauceBrowser.prototype.start = function() {
    ...
    self.debug('running');
    var browser = self.browser = wd.remote('localhost', 4723);
    ...
    self.controller = setup_test_instance(self._opt, function(err, url) {
        ...
        var init_conf = xtend({
            ...
            name: conf.name,
            tags: conf.tags || [],
            browserName: (conf.browser == 'iphone' ? 'safari' : 'browser'),
            platformVersion: conf.version,
            platformName: (conf.browser == 'iphone' ? 'iOS' : 'Android'),
        ...
        if (conf.browser == 'iphone') {
            init_conf.deviceName = 'iPhone 6'
        }
        if (conf.browser == 'android') {
            init_conf.deviceName = 'Android Emulator'
        }
